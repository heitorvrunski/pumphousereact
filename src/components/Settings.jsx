    ...
    useEffect(()=>{
        if(settings.length===0){
            ...
            config.value.forEach(element => {
                ...
                settingsAux.push({label:element.label,value:value,browseName:element.browseName})
              });
    ...
    return(
        <div className="row justify-content-center mx-2 d-flex">
            <div className="card m-2" style={{width:"320px"}}>
                ...
                <Table data={settings} classTable="table-sm d-flex overflow-auto h6" classInput="tableInput"   handleOptionOnChange={handleOptionOnChange}/>
                <button type="button" className="btn btn-principal my-2" onClick={handleSubmitChanges}>Update</button>
            </div>
        </div>
    )
}