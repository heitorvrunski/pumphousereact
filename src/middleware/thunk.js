...
const ApiNode = {
  GetSysConfig: () => (dispatch) => {
    axios
      .get(`http://${window.location.hostname}:${process.env.REACT_APP_NODE_PORT??3000}/node/api/sysConfig`, {
  ...
  PutSysConfig: (sysConfig) => (dispatch) => {
    axios
      .put(
        `http://${window.location.hostname}:${process.env.REACT_APP_NODE_PORT??3000}/node/api/sysConfig/${sysConfig.param}`,
        {
  ...
  PostSysConfig: (sysConfig) => (dispatch) => {
    axios
      .post(
        `http://${window.location.hostname}:${process.env.REACT_APP_NODE_PORT??3000}/node/api/sysConfig`,
        {
          param: sysConfig.param,
          value: sysConfig.value
        },
  ...
  RefreshToken: () => (dispatch) => {
    axios
      .get(`http://${window.location.hostname}:${process.env.REACT_APP_NODE_PORT??3000}/node/api/auth/refresh`, {
        withCredentials: true,
      })
      .then((resp) => resp.data)
 ...
 Logoff: () => (dispatch) => {
  ...
  GetUsers: () => (dispatch) => {
    axios
      .get(`http://${window.location.hostname}:${process.env.REACT_APP_NODE_PORT??3000}/node/api/Users`, {
        withCredentials: true,
      })
      .then((resp) => resp.data)
 ...
 GetLogs:  async function  (tail,module){
    ...
    await axios
      .get(`http://${window.location.hostname}:${process.env.REACT_APP_NODE_PORT??3000}/node/api/logDocker/${tail}/${module}`, {
        withCredentials: true,
      })
  ...
  GetLogsFile:  async function  (tail){
    ...
    await axios
      .get(`http://${window.location.hostname}:${process.env.REACT_APP_NODE_PORT??3000}/node/api/log/${tail}`, {
        withCredentials: true,
      })
      .then((resp) => {result = resp.data});
    return result
  ...
  CheckDocker:  async function  (){
    ...
    await axios
      .get(`http://${window.location.hostname}:${process.env.REACT_APP_NODE_PORT??3000}/node/api/cdocker`, {
        withCredentials: true,
      })
      .then((resp) => {
        if(resp.status===201)
  ...
  GetLogsRange:  async function  (type,module,date){
      ...
      await axios
        .post(
          `http://${window.location.hostname}:${process.env.REACT_APP_NODE_PORT??3000}/node/api/logDocker/range`,
          {
            type: type,
            module: module,
            date: date,
  ...
  CreateUser: (newUser) => (dispatch) => {
    axios
      .post(
        `http://${window.location.hostname}:${process.env.REACT_APP_NODE_PORT??3000}/node/api/Users`,
        {
          user: newUser.user,
  ...
  EditUser: (user) => (dispatch) => {
    axios
      .put(
        `http://${window.location.hostname}:${process.env.REACT_APP_NODE_PORT??3000}/node/api/Users/${user.user}`,
        {
          user: user.user,
          block: user.block,
          group: user.group,
  ...
  GetAlarmRange: async function  (startDate,endDate){
    ...
    await axios
      .post(
        `http://${window.location.hostname}:${process.env.REACT_APP_NODE_PORT??3000}/node/api/alarm/range`,
        {
          startDate: startDate,
          endDate: endDate,
        },
  ...
  GetAlarmOnline:  async function  (){
    var result;
    await axios
      .get(`http://${window.location.hostname}:${process.env.REACT_APP_NODE_PORT??3000}/node/api/alarm/online`, {
        withCredentials: true,
      })
      .then((resp) => {result = resp.data});
    return result
  ...
  RestartNode: () => {
    axios
      .get(
        `http://${window.location.hostname}:${process.env.REACT_APP_NODE_PORT??3000}/node/api/Docker/rbackend`,
        {
          withCredentials: true,
        }
      )
